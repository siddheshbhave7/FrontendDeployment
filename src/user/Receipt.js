...
const Receipt = () => {
  ...
  const getData = async () => {
    try {
      await axios
        .get(
          `http://ec2-23-20-146-4.compute-1.amazonaws.com:7020/ParkingPayment/booking/${vcookie.bookingid}`
        )
        .then((response) => {
  ...
  const payMoney = async () => {
    await axios
      .post(
        `http://ec2-23-20-146-4.compute-1.amazonaws.com:7020/ParkingPayment/getPayment/${cookie.userId}`,
        {
